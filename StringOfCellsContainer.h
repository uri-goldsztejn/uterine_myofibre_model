...
private:
	...
	static size_t numberOfCells_;  // The number of cells in the myofiber
	Stimulator stim_;			   // The stimulator used to stimulate the first cell in the myofiber.
	static vector<Cell> cells_;	   // This vector contains the cells of the myofiber

	// constant mechanical parameters used in each cell to calculate the force developed
	struct rparams
	{
		double kx1 = 12.5;
		double kx2 = 8.8;
...
public:
	...
	// Methods used to manage the sover used for the system of mechanical equations
	static int equationSystem(const gsl_vector * x, void *params, gsl_vector * f); // This function has to be static to be passed to solver
																				   // That requires that the member variables used in this function should be static (not elegant)
