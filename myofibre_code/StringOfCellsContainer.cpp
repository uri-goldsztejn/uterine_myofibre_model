...
// This method iterates over the container, it makes the calculations that involve parameters from multiple cells in the myofibre. Then, it calls the internal cells' iterate method.
void StringOfCellsContainer::iterateV2() {
	...
	// The depolarization of the first cell is initially caused by the stimulator and then propagates to the next cells in the myofibre.
	...
	for (it++; it != (cells_.end() -1); ++it) {
		...
		if (it == cells_.end() - 2) {					// The diffusion in the last cell is calculated using the backward derivative approximation. 
			previousPreviousVoltage = previousVoltage;	//Therefore, we need to save the voltage value of the two cells before the last one.
		}
	...
	long double diffusion_denom = chi*it->getLc()*it->getLc()*C*pow(10, -8); //get new cell's length

	//Iterate over each cell
	it->iterate(diffusion_num / diffusion_denom);

	
	//comment out the next line to maintain constant lengths and make the simulations faster (use for debugging only). Not calling the following function makes the simulations faster by ignoring the mechanical component of the model.
	// The results presented in the paper use the following function.
	solveMechanics(); // this function calculate the cells' lengths
}


// Returns a string consisting of the voltages of every cell, tab-separated
...
int StringOfCellsContainer::equationSystem(const gsl_vector * x, void * params, gsl_vector * f)
{
	...
	vector<double> y10; //the n equations represented by equation #15 in the manuscript 
	for (size_t i = 0; i < numberOfCells_; i++)
	{
	...
	vector<double> y11; //the n equations represented by equation #16 in the manuscript
	for (size_t i = 0; i < numberOfCells_; i++)
	{
	...
	vector<double> y12; //the n-1 equations represented by equation #18 in the manuscript
	...
	for (size_t i = 0; i < numberOfCells_; i++) // in the f vector the equations are stored in the following order: first the n equations #15 (i =1...n), then those numbered #16, then #18, and finally equation #19
	{
...
// Initialize the PDE solver from the GSL library.
...
// Returns the lengths of the cells in the container
...
// Returns the force developed by the myofibre. Form equation #17, we know that all the cells develop the same total force.
...
// Get the fraction of myosin in the force producing states. The method returns a string to write in the output files.
...
// Returns the current generated by the stimulator.
